...
test("when searching an empty file, an appropriate message is displayed", async ({
  page,
}) => {
  await page.getByLabel("Login").click();
  await page.getByLabel("Command input").click();
...
test("when loading a non-existing file, an appropriate message is displayed", async ({
  page,
}) => {
  await page.getByLabel("Login").click();
  await page.getByLabel("Command input").click();
  await page.getByLabel("Command input").fill("load_file non_existing_file");
  await page.getByLabel("Submit").click();
  await expect(page.getByText("File not found!")).toBeVisible();
});

test("when viewing without loading a file, an appropriate message is displayed", async ({
  page,
}) => {
  await page.getByLabel("Login").click();
  await page.getByLabel("Command input").click();
...
test("when searching without loading a file, an appropriate message is displayed", async ({
  page,
}) => {
  await page.getByLabel("Login").click();
  await page.getByLabel("Command input").click();
